...
function renderMatches(matches) {
	...
	// Trier : upcoming en premier, puis les passés par date desc
	const now = new Date();
	const upcoming = matches
	...
	const past = matches.filter((m) => m.type !== "upcoming");
	...
	track.innerHTML = sorted
		.map((m, idx) => {
			...
			if (m.type === "upcoming") {
				...
				const twitchBtn = m.twitchUrl
					? '<a class="twitch-link" href="' +
						m.twitchUrl +
						'" target="_blank" rel="noopener noreferrer"><svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor"><path d="M11.571 4.714h1.715v5.143H11.57zm4.715 0H18v5.143h-1.714zM6 0L1.714 4.286v15.428h5.143V24l4.286-4.286h3.428L22.286 12V0zm14.571 11.143l-3.428 3.428h-3.429l-3 3v-3H6.857V1.714h13.714z"/></svg>Twitch</a>'
					: "";
				return (
					'<div class="match-card upcoming">' +
					'<div class="match-upcoming-badge">À VENIR</div>' +
					...
					'<span class="match-score upcoming-vs">VS</span>' +
					'<span class="match-team right">' +
					(m.opponent || "???") +
					"</span>" +
					"</div>" +
					'<div class="match-countdown-mini" id="mcd-' +
					idx +
					'"></div>' +
			...
			const twitchBtn = m.twitchUrl
				? '<a class="twitch-link" href="' +
					m.twitchUrl +
					'" target="_blank" rel="noopener noreferrer"><svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor"><path d="M11.571 4.714h1.715v5.143H11.57zm4.715 0H18v5.143h-1.714zM6 0L1.714 4.286v15.428h5.143V24l4.286-4.286h3.428L22.286 12V0zm14.571 11.143l-3.428 3.428h-3.429l-3 3v-3H6.857V1.714h13.714z"/></svg>VOD</a>'
				: "";
			return (
	...
	// Lancer les mini-countdowns pour les matchs à venir
	sorted.forEach((m, idx) => {
		if (m.type !== "upcoming") return;
		const el = document.getElementById("mcd-" + idx);
		if (!el) return;
		const target = new Date(m.date + "T" + (m.time || "00:00"));
		function tickMini() {
			const diff = target - new Date();
			if (diff <= 0) {
				el.innerHTML = '<span class="mcd-live">🔴 EN COURS</span>';
				return;
			}
			const d = Math.floor(diff / 86400000);
			const h = Math.floor((diff % 86400000) / 3600000);
			...
			el.innerHTML =
				'<div class="mcd-wrap">' +
				'<div class="mcd-unit"><span class="mcd-num">' +
				String(d).padStart(2, "0") +
				'</span><span class="mcd-lbl">j</span></div>' +
				'<span class="mcd-sep">:</span>' +
				'<div class="mcd-unit"><span class="mcd-num">' +
				String(h).padStart(2, "0") +
				'</span><span class="mcd-lbl">h</span></div>' +
				'<span class="mcd-sep">:</span>' +
				'<div class="mcd-unit"><span class="mcd-num">' +
				String(min).padStart(2, "0") +
				'</span><span class="mcd-lbl">m</span></div>' +
				'<span class="mcd-sep">:</span>' +
				'<div class="mcd-unit"><span class="mcd-num">' +
				String(s).padStart(2, "0") +
				'</span><span class="mcd-lbl">s</span></div>' +
				"</div>";
			cdIntervals.push(setTimeout(tickMini, 1000));
		}
		tickMini();
	});
